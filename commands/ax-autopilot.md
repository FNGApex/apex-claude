...
3. **Implement.** Run the `/ax-implement` loop. Address EVERY reviewer finding in-iteration — defer nothing. If stuck, dispatch `ax-strategist` (read-only) for root-cause reasoning before retrying.
